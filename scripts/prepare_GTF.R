...
# Julien Wollbrett
# modified 07/01/2019: Correct a bug of scientific notation for start or stop of intergenic region. We disabled scientific notation.
...
## Invoking:
# R CMD BATCH --no-save --no-restore '--args gene_gtf_path="gene_gtf_path" genome_fasta_path="genome_fasta_path" N_block_size = 31 N_proportion = 0.05 output_gtf_file_path="output_gtf_file_path"' prepare_gtf.R Rout_path

## Example:
# R CMD BATCH --no-save --no-restore '--args gene_gtf_path="~/Desktop/RNAseq/pipeline_Ensembl_73/Mus_musculus.GRCm38.73.gtf.gz" genome_fasta_path="~/Desktop/RNAseq/pipeline_Ensembl_73/Mus_musculus.GRCm38.73.genome.fa" N_block_size = 31 N_proportion = 0.05 output_gtf_file_path="~/Desktop/RNAseq/pipeline_Ensembl_73/gtf_folder/Mus_musculus.GRCm38.73"' prepare_GTF.R
...
# "output_gtf_file_path"    - full path to output custom gtf file
...
if (!exists("output_gtf_file_path")){ stop("output_gtf_file_path not defined") }
...
## This object will contain the coordinates of the selected intergenic regions without N
...
colnames(summary_N_removal) <- c("intergenic_regions", "total_bp", "total_N", "percentage_N")
...
summary_N_removal[,"percentage_N"] <- summary_N_removal[,"total_N"] / summary_N_removal[,"total_bp"]

####################################
## Output:
## GTF file with both genic exons and intergenic regions
write.table(rbind(gene_gtf_exon, intergenic_regions_gtf), file=output_gtf_file_path, sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)

## Table summaryzing the step of block of Ns removal
write.table(summary_N_removal, file=paste(sub(pattern = "(.*)\\..*$", replacement = "\\1", output_gtf_file_path), "_Nremoval_summary.tsv", sep=""), sep="\t", row.names=TRUE, col.names=TRUE, quote=FALSE)
